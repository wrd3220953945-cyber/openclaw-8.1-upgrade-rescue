...
→ **完整修复流程（含备份与验证）：[第四节](#四完整修复流程)** · 一键清单：[附录](#附一键排查清单)

如果你正打算「删掉重装、重新配置所有 Agent」——**先停手，读完这篇**。

另外有一个**不报错的隐蔽后果**：8.1 改了 Agent 工作目录的解析规则，部分 Agent 会被静默换到一个空目录，看上去像「记忆全丢了」（实际没丢）。修好启动后请务必看一眼：[坑 6](#坑6agent-工作目录被静默重定向最隐蔽的一个)。
